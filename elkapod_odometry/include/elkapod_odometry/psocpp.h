...
class NoCallback {
 public:
  ...
  bool operator()(const Index, const Matrix &, const Vector &, const Index) const { return true; }
};
...
class ConstantWeight {
 ...
 public:
  ...
  ConstantWeight(const Scalar weight) : weight_(weight) {}

  Scalar operator()(const Index, const Index) const { return weight_; }
};
...
class ExponentialDecrease3 {
 ...
 public:
  ...
  ExponentialDecrease3(const Scalar minval, const Scalar maxval, const Scalar d1, const Scalar d2)
...
#pragma omp parallel for num_threads(threads_)
    for (Index i = 0; i < particles.cols(); ++i) fvals(i) = objective_(particles.col(i));
  }
  ...
  void calculateVelocities(const Matrix &particles, const Matrix &bestParticles, const Index gbest,
                           const Index iteration, Matrix &velocities) {
  ...
  Result _minimize(const Matrix &bounds, Matrix &particles) {
    ...
    while ((maxIt_ == 0 || iterations < maxIt_) && fchange > feps_ && xchange > xeps_) {
      ...
      if (verbosity_ > 0) {
        ...
        if (verbosity_ > 2) ss << "    callback=" << (callbackResult ? "true" : "false");
 ...
 public:
  ...
  void setMinParticleChange(const Scalar change) { xeps_ = change; }
  ...
  void setMinFunctionChange(const Scalar change) { feps_ = change; }
  ...
  void setPhiParticles(const Scalar phip) { phip_ = phip; }
  ...
  void setPhiGlobal(const Scalar phig) { phig_ = phig; }
  ...
  void setMaxVelocity(const Scalar maxvel) { maxVel_ = maxvel; }
  ...
  Result minimize(const Matrix &bounds, const Index cnt) {
    ...
    if (bounds.rows() != 2) throw std::runtime_error("bounds has not exactly 2 rows (min, max)");
    for (Index i = 0; i < bounds.cols(); ++i) {
      if (bounds(0, i) >= bounds(1, i)) throw std::runtime_error("bounds min is greater than max");
    }
