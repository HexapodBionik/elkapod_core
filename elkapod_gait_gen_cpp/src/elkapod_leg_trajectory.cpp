#include "../include/elkapod_gait_gen_cpp/elkapod_leg_trajectory.hpp"

Trajectory::Trajectory(const double timestep) { this->timestep = timestep; }

void Trajectory::addPoint(const Vec3& p) { points.push_back(p); }

Vec3 Trajectory::at(size_t i) const { return points[i]; }
...
size_t Trajectory::size() const { return points.size(); }

double Trajectory::totalDuration() const { return points.size() * timestep; }
...
Trajectory HopLegPlanner::plan(const Vec3& start, const Vec3& goal, double duration, double hz) {
  ...
  const double path_length =
      std::sqrt(std::pow(start[0] - goal[0], 2) + std::pow(start[1] - goal[1], 2) +
                std::pow(start[2] - goal[2], 2));
  ...
  auto z_func = [start, goal, R](double s) {
    return start[2] + (goal[2] - start[2]) * s + R * std::sin(M_PI * s);
  };
...
bool TrajectoryExecutor::hasNext() const { return current_step < max_steps; }
