...
void elkapod_comm::SpiDevice::setMode(uint8_t mode) { mode_ = mode; }

void elkapod_comm::SpiDevice::setSpeed(uint32_t hz) { speed_ = hz; }

void elkapod_comm::SpiDevice::setBitsPerWord(uint8_t bits) { bits_ = bits; }

void elkapod_comm::SpiDevice::applySettings() {
  if (ioctl(fd_, SPI_IOC_WR_MODE, &mode_) < 0) throw std::runtime_error("Failed to set SPI mode");
