...
static LibSerial::BaudRate convert_baud_rate(int baud_rate) {
  switch (baud_rate) {
    ...
    default:
      std::cout << "Error! Baud rate " << baud_rate << " not supported! Default to 115200"
                << std::endl;
      return LibSerial::BaudRate::BAUD_115200;
  }
}

ElkapodComm::ElkapodComm(std::unique_ptr<UARTDevice> uart, std::unique_ptr<SpiDevice> spi)
    : uart_(std::move(uart)), spi_(std::move(spi)) {}
...
void ElkapodComm::sendSystemShutdownCommand() { uart_->sendSystemShutdownCommand(); }

bool ElkapodComm::checkConnection() { return uart_->checkConnection(); }

UARTDevice::UARTDevice(const std::string device_path, const uint32_t baudrate, const int timeout_ms)
    : device_path_(device_path), baudrate_(baudrate), timeout_ms_(timeout_ms) {
