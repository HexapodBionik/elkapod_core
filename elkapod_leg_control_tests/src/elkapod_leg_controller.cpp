...
void ElkapodLegPublisher::topicCallback(std_msgs::msg::Float64MultiArray::SharedPtr msg){
    ...
    */
    ...
    for(int i = 0; i < 6; ++i){
        ...
        std::string msg = std::format("Point for leg {} x: {:.4f} y: {:.4f} z: {:.4f}", i+1, input[0], input[1], input[2]);
        ...
        std::string msg2 = std::format("Angles for leg {} theta0: {:.3f} theta1: {:.3f} theta2: {:.3f}", i+1, output_msg.data[i*3], output_msg.data[i*3+1], output_msg.data[i*3+2]);
