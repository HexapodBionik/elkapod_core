...
controller_interface::CallbackReturn ElkapodIKController::on_error(const rclcpp_lifecycle::State &)
{
  return controller_interface::CallbackReturn::SUCCESS;
}
...
ElkapodIKController::on_export_reference_interfaces()
{
    ...
    for (std::size_t i = 0; i < params_.joints.size(); ++i) {
      ...
      refs.emplace_back(
        std::format("{}/leg{}_{}", ctrl_name, i / 3 + 1, coordinate),
        "point",          
...
PLUGINLIB_EXPORT_CLASS(
  elkapod_ik_controller::ElkapodIKController, controller_interface::ChainableControllerInterface)