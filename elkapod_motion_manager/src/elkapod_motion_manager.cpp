...
ElkapodMotionManager::ElkapodMotionManager(): Node("elkapod_motion_manager"){
    ...
    this->walk_enable_service_ = this->create_service<std_srvs::srv::Trigger>("/motion_manager_walk_enable", std::bind(&ElkapodMotionManager::walkEnableServiceCallback, this, _1, _2), 10, my_group_);
    this->walk_disable_service_ = this->create_service<std_srvs::srv::Trigger>("/motion_manager_walk_disable", std::bind(&ElkapodMotionManager::walkDisableServiceCallback, this, _1, _2), 10, my_group_);

    this->gait_enable_publisher_ = this->create_client<std_srvs::srv::Trigger>("/gait_gen_enable", 10, my_group_);
    this->gait_disable_publisher_ = this->create_client<std_srvs::srv::Trigger>("/gait_gen_disable", 10, my_group_);
...
rclcpp_action::GoalResponse ElkapodMotionManager::transition_action_handle_goal([[maybe_unused]]const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const TriggerAction::Goal> goal){
    if(executor_enable_){
      RCLCPP_INFO(this->get_logger(), "Robot is currently transitioning to next state thus new goal has been rejected!");
...
void ElkapodMotionManager::transition_action_handle_accepted(const std::shared_ptr<GoalHandleTriggerAction> goal_handle){
    auto execute_in_thread = [this, goal_handle](){return this->transition_action_execute(goal_handle);};
    std::thread{execute_in_thread}.detach();
}

void ElkapodMotionManager::transition_action_execute(const std::shared_ptr<GoalHandleTriggerAction> goal_handle){
    planning_method_();
...
void ElkapodMotionManager::walkEnableServiceCallback([[maybe_unused]]const std::shared_ptr<std_srvs::srv::Trigger_Request> request, std::shared_ptr<std_srvs::srv::Trigger_Response> response){ 
  if(!executor_enable_ && state_ == State::IDLE){
    ...
    if(future.wait_for(5s) == std::future_status::ready){
      ...
      RCLCPP_WARN(this->get_logger(), "Gait generator hasn't been enabled due to some error! Staying in IDLE state.");
      response->success= false;
    }
  }
  else{
    RCLCPP_INFO(this->get_logger(), "Can't transition to WALKING state because ongoing transition and/or manager is in state which prevents such transition");
    response->success= false;
  }
}

void ElkapodMotionManager::walkDisableServiceCallback([[maybe_unused]]const std::shared_ptr<std_srvs::srv::Trigger_Request> request, std::shared_ptr<std_srvs::srv::Trigger_Response> response){ 
  if(!executor_enable_ && state_ == State::WALKING){
    ...
    if(future.wait_for(5s) == std::future_status::ready){
      ...
      RCLCPP_WARN(this->get_logger(), "Gait generator hasn't been enabled due to some error! Staying in WALKING state.");
      response->success= false;
    }
  }
  else{
    RCLCPP_INFO(this->get_logger(), "Can't transition to IDLE state because ongoing transition and/or manager is in state which prevents such transition");
    response->success= false;
  }
}
